...
export default class RestrictedDraggable {
    ...
    createElement(id: string, layout: { top: number; left: number; width: number; height: number; }) {
    ...
    setDragBounds(restrictionBoundingBox: { top: number; right: number; bottom: number; left: number; } | undefined) {
    ...
    onMouseMove(e: MouseEvent) {
        if(this.isMoving){
            ...
            const isWithinHorizontalBounds = newXValue >= this.leftBoundary && newXValue + this.element.offsetWidth <= this.rightBoundary;

            const isWithinVerticalBounds = newYValue >= this.topBoundary || newYValue + this.element.offsetHeight <= this.bottomBoundary;
